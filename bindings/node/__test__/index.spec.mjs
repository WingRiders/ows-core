...
describe('@open-wallet-standard/core', () => {
  ...
  it('derives addresses for all chains', () => {
    const phrase = generateMnemonic(12);
    for (const chain of ['evm', 'solana', 'sui', 'bitcoin', 'cosmos', 'tron', 'ton', 'filecoin', 'xrpl', 'nano', 'near', 'midnight']) {
  ...
  it('creates a universal wallet with accounts for all chains', () => {
    ...
    const midnight = wallet.accounts.find((a) => a.chainId === 'midnight:mainnet');
    assert.ok(midnight, 'expected midnight:mainnet account');
    assert.ok(midnight.address.startsWith('mn_addr1'), 'Midnight account uses unshielded address');
    assert.ok(
      !wallet.accounts.some((a) => a.chainId === 'midnight:preview'),
      'preview address is derived at use time, not stored',
    );
  ...
  it('imports a secp256k1 private key with all 13 accounts', () => {
    ...
    assert.equal(wallet.accounts.length, 13, 'should have one account per chain type');
  ...
  it('signs transactions on all chains', () => {
    ...
    for (const chain of ['evm', 'solana', 'sui', 'bitcoin', 'cosmos', 'tron', 'ton', 'filecoin', 'xrpl', 'nano', 'near']) {
      ...
      assert.ok(result.signature.length > 0, `expected signature for ${chain}`);
    }
