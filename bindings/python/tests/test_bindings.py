...
def test_derive_address_all_supported_chains():
    phrase = ows.generate_mnemonic(12)
    for chain in ["evm", "solana", "sui", "bitcoin", "cosmos", "tron", "ton", "filecoin", "xrpl", "nano", "near", "midnight"]:
...
def test_create_and_list_wallets(vault_dir):
    ...
    assert any(c.startswith("midnight:") for c in chain_ids)
    assert "midnight:mainnet" in chain_ids
    assert "midnight:preview" not in chain_ids
