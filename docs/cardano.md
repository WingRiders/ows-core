...
endpoint and pass it through. Which chains need one is a property of the signer,
...
was made `pub(crate)`-visible for this. For every chain that does not ask for a URL it
...
3. If there is anything to resolve, an RPC URL is **required** (else
   `InvalidMessage`); `resolve_cardano_provider` selects Koios or Blockfrost from the
   URL (see [§1.4](#14-rpc-configuration-koios-and-blockfrost)). Inputs are resolved
   *not* by asking the provider for UTxO values, but by fetching the **CBOR of each
...
keeps the one trust model (see
[§4.2](#42-parsing-and-input-resolution-cardanorpcproviderfetch_txs_cbor)).

Collateral inputs join the **same** resolution pass as the spent inputs, de-duplicated
by source transaction hash, so they add nothing when they reference a transaction the
inputs already pull in, and otherwise cost what one more hash costs on the configured
provider: a slot in Koios's ten-hash `tx_cbor` chunk, so an extra call only when the
unique hashes cross a multiple of ten; one `GET` on Blockfrost, which serves CBOR per
hash. They are subject to the same fail-closed rule: a collateral input the provider
cannot return aborts context construction. `chain_extra` is omitted entirely when a
transaction declares no collateral, which is every non-script transaction.
...
12. `make_transaction_context` parses an unsigned Cardano transaction, resolves its
    ...
    address; declared collateral is resolved in the same batch and reported under
    `chain_extra.collateral_effects`, net of `collateral_return`, and is absent for
    a transaction without collateral; it errors when the RPC URL is missing for a
    transaction with inputs, or when the provider returns incomplete UTxO data.
...
- **What the RPC provider learns.** Building the policy context is the first thing in
  OWS that reaches the network on a signing path, so it is worth being exact about
  what leaves the machine. **The transaction being signed is never sent.**
  `make_transaction_context` asks the configured provider for the CBOR of
  transactions that are *already on chain*, by hash — the hashes the unsigned
  transaction's inputs and collateral reference (Koios `POST {rpc}/tx_cbor`,
  Blockfrost `GET {rpc}/txs/{hash}/cbor`). Signing itself is local; no key material,
  and no part of the transaction under construction, is transmitted — the HTTP client
  lives in `ows-core`, not in `ows-signer`, which never holds a network dependency of
  its own. What the provider (and anything on the network path) can observe is the
  set of source transaction hashes and the requester's IP, from which it can infer
  which UTxOs the wallet is about to spend and correlate requests over time.
  Broadcast is the separate step that does transmit the finished transaction (Koios
  `POST {rpc}/submittx`, Blockfrost `POST {rpc}/tx/submit`), as any submission path
  must. A deployment that treats that inference as sensitive should point `rpc`
  config at a provider it operates (see
  [§1.4](#14-rpc-configuration-koios-and-blockfrost)); the keyless Koios default
  trades this for needing no account and storing no credential, while Blockfrost ties
  the same requests to a `project_id`.
...
- `ows/crates/ows-signer/src/chains/cardano.rs` — `CardanoSigner`, CIP-1852 path helpers,
  ...
  `resolve_cardano_provider` and `CardanoRpcProvider::fetch_txs_cbor`, books
  withdrawals and certificate deposits/refunds against the reward address, and
  reports declared collateral under `chain_extra.collateral_effects`).
...
- `reqwest = "0.12"` (`json`, `rustls-tls`, no default features; `blocking` in
  ...
  (`ows/crates/ows-core/Cargo.toml`, `ows/crates/ows-pay/Cargo.toml`). It is a
  dependency of `ows-core`, not of `ows-signer`, so the crate that handles key
  material has no network dependency of its own; what the RPC layer does and does
  not send is spelled out under "What the RPC provider learns" in Security
  considerations.
...
- **Policy context** (`cardano.rs`, `cardano_rpc/`): `make_transaction_context` is
  ...
  the wallet's own addresses and to a third party. Three collateral shapes are
  covered too: collateral with no `collateral_return` (the whole collateral input is
  at risk, and none of it appears in `effects`), collateral with a partial return
  (only the remainder is at risk), and a transaction with no collateral at all
  (`chain_extra` is absent). Three staking shapes are covered
